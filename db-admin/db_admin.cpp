...
std::vector<Column> get_table_columns(sqlite3* db, const std::string& table_name) {
    std::vector<Column> columns;
    sqlite3_stmt *stmt;
...
std::vector<std::map<std::string, std::string>> get_table_data(sqlite3* db, const std::string& table_name, const std::vector<Column>& columns) {
    std::vector<std::map<std::string, std::string>> records;
    sqlite3_stmt *stmt;
    
    std::string sql = "SELECT * FROM " + table_name + " LIMIT 100;";
...
std::string generate_main_page(sqlite3* db) {
    ...
    for (const auto& table : tables) {
        html << "<li><a href='/table?name=" << table << "'>" << table << "</a></li>";
    }
...
std::string generate_table_view(sqlite3* db, const std::string& table_name) {
    ...
    std::vector<std::map<std::string, std::string>> records = get_table_data(db, table_name, columns);
    ...
    html << "</div>"
         ...
         << "<div class='actions'>"
         << "<button onclick=\"location.href='/insert?table=" << table_name << "'\">Add New Record</button>"
         << "<button onclick=\"location.href='/export?table=" << table_name << "'\">Export CSV</button>"
         << "</div>"
         << "<table><tr>";
    ...
    for (const auto& record : records) {
        ...
        html << "<td>"
             << "<a href='/edit?table=" << table_name << "&id=" << record.at("id") << "'>Edit</a> | "
             << "<a href='/delete?table=" << table_name << "&id=" << record.at("id") << "' onclick='return confirm(\"Are you sure?\")'>Delete</a>"
             << "</td>";
